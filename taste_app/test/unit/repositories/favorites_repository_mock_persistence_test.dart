...
void main() {
  group('Mock Favorites Persistence Tests', () {
    ...
    test('should persist mock favorite exclusions in SharedPreferences', () async {
      // Arrange
      const mockRestaurantId = 'mock_1';
      final prefs = await SharedPreferences.getInstance();
      
      // Act - Simular a adição de um favorito excluído
      final currentExcluded = prefs.getStringList(excludedMockFavoritesKey) ?? [];
    ...
    test('should filter excluded mock favorites from list', () async {
      ...
      final excludedIds = prefs.getStringList(excludedMockFavoritesKey) ?? [];
      final filteredIds = allMockIds.where((id) => !excludedIds.contains(id)).toList();
    ...
    test('should restore mock favorite by removing from excluded list', () async {
      // Arrange
      ...
      final finalExcludedIds = prefs.getStringList(excludedMockFavoritesKey) ?? [];
    ...
    test('should persist data between app sessions', () async {
      ...
      final excludedIds = newPrefs.getStringList(excludedMockFavoritesKey) ?? [];
      expect(excludedIds, contains(mockRestaurantId));
    });
  });
}