...
class OrderService {
  ...
  Future<OrderModel> createOrder({
    ...
    try {
      ...
      if (cart.restaurant!.minOrderValue != null &&
          cart.subtotal < cart.restaurant!.minOrderValue!) {
        throw Exception(
      ...
      throw Exception(
          'Erro ao criar pedido: ${e.toString().replaceAll('Exception: ', '')}');
    }
  }
  ...
  Future<List<OrderModel>> getOrderHistory() async {
    ...
    return _orders
        .where((order) => finishedStatuses.contains(order.status))
        .toList();
  }
  ...
  DateTime _calculateEstimatedDeliveryTime(
    ...
    return now.add(const Duration(minutes: preparationTime + deliveryTime));
  }
