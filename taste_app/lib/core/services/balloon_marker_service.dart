...
class BalloonMarkerService {
  ...
  static const Color _selectedBorderColor =
      Color(0xFF8B5CF6); // Exact React color
  ...
  static Future<gmaps.BitmapDescriptor> createBalloonMarker({
    ...
    final cacheKey =
        '${emoji}_${isSelected}_${isCluster}_${clusterCount ?? ''}_${category ?? ''}';
  ...
  static void _drawShadow(
      Canvas canvas, double size, double totalWidth, double totalHeight) {
    ...
    // Shadow circle
    shadowPath.addOval(Rect.fromLTWH(_shadowOffset, _shadowOffset, size, size));
  ...
  static void _drawBalloon(Canvas canvas, double size, bool isSelected,
      bool isCluster, String? category) {
  ...
  static void _drawTail(Canvas canvas, double size, double totalWidth,
    ...
    const squareSize = _tailWidth * 0.7; // Smaller square for better proportion
  ...
  static Future<gmaps.BitmapDescriptor> createSelectedMarker(String emoji,
  ...
  static gmaps.BitmapDescriptor _createWebFallbackMarker({
    ...
    debugPrint(
        '🎨 Web fallback marker - Emoji: $emoji, Hue: $hue, Selected: $isSelected, Cluster: $isCluster');
